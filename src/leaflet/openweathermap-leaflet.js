L.OWM = L.TileLayer.extend({
	options: {
		appId: '', /* Get your own AppId at https://www.openweathermap.org/appid */
...
L.OWM.Current = L.Layer.extend({
		...
		for (var key in stations) {
			...
			if (_this.options.popup) {
				if (_this.options.popupFunction != null && typeof _this.options.popupFunction == 'function') {
					...
					//_this._getForecastData(stations[key]);
				}
			}
			...
			if (markerWithPopup != null && markerWithPopup.options.owmId == marker.options.owmId) {
				_this._getForecastData(stations[key]);
			}
		}
		...
		*/
		...
		if (typeof station.main != 'undefined') {
			...
			if (this.options.showTempMinMax) {
				if (typeof station.main.temp_max != 'undefined' && typeof station.main.temp_min != 'undefined') {
					txt += '<div class="owm-popup-detail">'
						+ this.i18n('minmax_temp', 'Min/Max Temp')
						+ ': '
		...
		if (station.rain != null && typeof station.rain != 'undefined' && typeof station.rain['1h'] != 'undefined') {
			txt += '<div class="owm-popup-detail">'
				...
				+ ': ' + station.rain['1h'] + '&nbsp;mm</div>';
		}
		if (station.snow != null && typeof station.snow != 'undefined' && typeof station.snow['1h'] != 'undefined') {
			txt += '<div class="owm-popup-detail">'
				+ this.i18n('snow_1h', 'Snow (1h)')
				+ ': ' + station.snow['1h'] + '&nbsp;mm</div>';
		}
		if (typeof station.wind != 'undefined') {
			if (typeof station.wind.speed != 'undefined') {
				...
				if (this.options.showWindSpeed == 'beaufort' || this.options.showWindSpeed == 'both') {
					txt += this.i18n('windspeed', 'Wind Speed')
			...
			if (typeof station.wind.deg != 'undefined') {
				txt += '<div class="owm-popup-detail">';
				txt += this.i18n('winddirection', 'Wind Direction') + ': ';
	...
	_getForecastData: function(station) {
		fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${station.coord.Lat}&lon=${station.coord.Lon}&exclude=minutely,hourly&units=metric&appid=${this.options.appId}`)
		.then(res => res.json())
		.then(data => {
			this._showForecastData(data)
		})
	},

	_showForecastData: function(data) {
		//let {humidity,pressure,temp,wind_deg,wind_speed} = data.current;
...
L.OWM.Utils = {
	...
	i18n: {
		...
		it: {
			...
			, minmax_temp: 'Temp. min / max '
			, wind: 'Vento'
			, windgust: 'Raffica'
			, windspeed: 'Forza del vento'
			, winddirection: 'Direzione'
			, rain_1h: 'Pioggia'
			, humidity: 'Umidità'
			, pressure: 'Pressione'
